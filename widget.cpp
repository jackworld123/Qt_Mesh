...
/********************************************************************************/
Widget::Widget(QWidget *parent) ://构造函数后加冒号是初始化表达式
    QMainWindow(parent),//如果有参数parent不为空，则MyFirstWidget会成为该指针所指窗口的子窗口
    ui(new Ui::Widget)//创建Ui::MyFirstWidget界面对象，并将指针赋值给ui，创建的对象会在析构函数中释放
{
...
// 2)表示“域操作符”.例：声明了一个类A，类A里声明了一个成员函数void f()，但没有在类的声明里给出f的定义，那么在类外定义f时,
...
void Widget::createMenuBar()
{
    ...
    mainMenu = menuBar()->addMenu(tr("主机面"));
    realstaMenu = menuBar()->addMenu(tr("实时状态"));
    basesettingMenu = menuBar()->addMenu(tr("设备基本设置"));
    advsettingMenu = menuBar()->addMenu(tr("设备高级设置"));
    parsettingMenu = menuBar()->addMenu(tr("调度参数设置"));
    videoMenu = menuBar()->addMenu(tr("录像调阅"));
    voiceMenu = menuBar()->addMenu(tr("语音"));
    SpecMenu = menuBar()->addMenu(tr("频谱"));
    lookMenu = menuBar()->addMenu(tr("外观设置"));
    aboutMenu = menuBar()->addMenu(tr("关于"));
    exitMenu = menuBar()->addMenu(tr("退出"));
